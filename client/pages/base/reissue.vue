...
<template>
    <div class="identity-page">
        ...
        <lost-reissue-business v-if="showIndex==1" :isLost="false" :isMakeFina="isMakeFina"></lost-reissue-business>
...
import LostReissueBusiness from '~/components/business/lostReissueBusiness.vue'
    ...
    methods:{
        ...
        ConfirmBtn(isMakeFina) {
            if (isMakeFina==1) {
                // C端交易功能点编号
                //目前传的是挂失同号制卡
                this.$store.commit('SET_SERVICE_RESULT', {"busiCode": this.$SysCfg.BUSI_CODES["cardBusi"]["sameNumReissue"]})
                console.log("C端交易功能点编号 >> ", this.$store.state.service_result)
            } else {
                this.$store.commit('SET_SERVICE_RESULT', {"busiCode": this.$SysCfg.BUSI_CODES["cardBusi"]["diffNumReissue"]})
                console.log("C端交易功能点编号 >> ", this.$store.state.service_result)
            }
